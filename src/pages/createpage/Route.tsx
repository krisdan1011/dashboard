...
function enableDiagnostics(query: Query, user: User): Promise<any> {
  console.info("ENABLING DIAGNOSTICS FOR SOURCE " + query.id + " AND USER " + user.userId);
  let source: Source;
  return SourceService.getSourceObj(query.id)
    .then(function(sourceFetched: Source){
      source = sourceFetched;
      return SpokeService.fetchPipe(user, source);
    }).then(function(pipe: any){
      if (!pipe.diagnosticsKey) {
        pipe.diagnosticsKey = query.key;
        return SpokeService.savePipe(user, source, pipe.http, true);
      }
      return;
    });
}
    ...
    reroute(props: CreateOrRouteProps): Promise<any> {
        ...
        const promise = checkParams(query)
            .then(function (query: Query) {
                ...
                return enableDiagnostics(query, currentUser).then(function(){
                  return result;
                });
